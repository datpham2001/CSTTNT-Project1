...
# Create class Queue to use find path by BFS
class Queue():
    def __init__(self):
        self.frontier = []
...
# Make wall from cells with signal "x"
def makeWall(maze):
    wall = []
    ...
    while True:
        ...
        for neighbor_node in neighbors(node):
            if neighbor_node.state not in visited and not fringe.contains_state(neighbor_node.state):
                if neighbor_node.state in gates.keys():
                    neighbor_node.state = gates[neighbor_node.state]
...
'''
gates, maze = support.readFile('./maze_with_gate3.txt')
...
# Solution
path, num_explored = solveMaze(maze, start, end)

# Visualize maze and path (change type = 1 or 2 to be suitable for map size)
support.visualize_maze(maze, gates, start, end, path)

print('Chi phi duong di: ', len(path))
print('Tong so cac trang thai da xet: ', num_explored)
